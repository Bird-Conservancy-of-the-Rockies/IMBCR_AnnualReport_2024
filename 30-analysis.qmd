...
where $d_{[low]}$ and $d_{[high]}$ are the bounds for distance bin $d$, and $σ$ is the half-normal shape parameter. Because of the lack of an analytical solution to the integral of the hazard rate function, we calculated $p_{[pd],td}$ at the midpoint, $d_{[mid]}$, of each distance class:

$$p_{[pd],td}=1-exp\biggl(-\biggl(\frac{d_{[mid]}}{h}\biggr)^b\biggr),$$ {#eq-abundance_p_pd_HZ}

where $h$ and $b$ are estimated parameters for the hazard function. For perceptibility models (@eq-abundance_p_pd_HN and @eq-abundance_p_pd_HZ), shape parameters were modeled as constant across space and time. Previously, we had modeled perceptibility shape parameters as variable across years for species with sufficient data. For the sake of computational efficiency and considering distance sampling benefits from pooling robustness (Buckland et al. 2001, 2008), we simplified to constant-perceptibility models starting in 2024. We allowed availability to vary by year and BCR:

$$logit(p_{[ar],tl}) \sim Normal(μ_{[p_{ar}]},SD_{[p_{ar}]}^2).$$ {#eq-abundance_p_ar}

Priors for annual variation in availability are

$$μ_{[p_{ar}]} \sim Normal(0,1),$$ {#eq-abundance_mu_par}

<center>and</center>

$$SD_{[p_{ar}]} \sim gamma(1,1).$$ {#eq-abundance_SD_par}
...
where $p_{[R],tl}$ is species detectability for a single 2-min time removal bin. We model variability in species detectability here ($p_{[R],tl}$) in the same manner as the availability component of the abundance model ($p_{[ar]}$ in @eq-abundance_p_ar, @eq-abundance_mu_par, and @eq-abundance_SD_par), including the shorter 5-min survey period in 2008-2009 (see explanation following @eq-abundance_pi_a_cell_c).
